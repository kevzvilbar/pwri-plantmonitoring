...
export interface ReportRunningExemptionOpts {
  trainId: string;
  plantId: string;
...
/**
 * Headless version of the exemption filing for callers that already own a
 * supabase client + query client (e.g. usePretreatmentActions on the RO Train
 * log page). Same writes as the useReportTrainRunning() hook below.
 */
export async function reportTrainRunningExemption(
  supabase: any,
  qc: { invalidateQueries: (opts: { queryKey: string[] }) => void },
  opts: ReportRunningExemptionOpts,
): Promise<void> {
...
export function useReportTrainRunning() {
  ...
  const reportRunning = async (opts: ReportRunningExemptionOpts) => {
    await reportTrainRunningExemption(supabase, qc, opts);
  };
