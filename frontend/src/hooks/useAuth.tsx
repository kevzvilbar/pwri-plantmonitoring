...
interface AuthContextValue {
  ...
  isAdmin: boolean;
  isManager: boolean;
  /** True when the user holds Admin or Data Analyst role.
   *  Data Analysts can normalize, tag, retract, and run regression — but
   *  cannot hard-delete records or assign roles (Admin-only). */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Data Analyst: second-highest authority. Can normalize/tag/retract but not
  // hard-delete or assign roles. Admin inherits all Data Analyst permissions.
  const isDataAnalyst = isAdmin || roles.includes('Data Analyst');
  ...
  return (
    <AuthContext.Provider value={{ session, user, profile, activeOperator, activeOperatorId, roles, isAdmin, isManager, isDataAnalyst, loading, isRecovery, refreshProfile, signOut }}>
      {children}
