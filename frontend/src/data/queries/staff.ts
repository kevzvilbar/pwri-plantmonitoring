...
export async function fetchKpiReadings(range: 'today' | number): Promise<{
  ...
  const [wellReadings, locReadings, roReadings, meterReadings, powerReadings, chemReadings, blendingReadings] = await Promise.all([
    ...
    supabase.from('power_readings').select('plant_id, reading_datetime, recorded_by, daily_solar_kwh, daily_grid_kwh, is_estimated').gte('reading_datetime', since),
    supabase.from('chemical_dosing_logs').select('plant_id, log_datetime, recorded_by').gte('log_datetime', since),
    supabase.from('blending_events').select('plant_id, well_id, event_date, is_estimated').gte('event_date', since.slice(0, 10)),
  ]);
