...
export function ReconciliationHealthCard({ plantIds }: { plantIds: string[] }) {
  ...
  const { rows, isLoading } = useReconciliationHealthTotals(plantIds);
  ...
  return (
    <Card className="rounded-2xl p-3.5 transition-all hover:border-border/90" data-testid="reconciliation-health-card">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          <Scale className="h-4 w-4 text-primary" />
          <span className="text-sm font-semibold">Permeate vs Product Reconciliation</span>
        </div>
      ...
      {isLoading ? (
        <div className="h-[110px] flex items-center justify-center text-xs text-muted-foreground">Loading…</div>
      ) : rows.length === 0 ? (
        <div className="h-[90px] flex items-center justify-center text-center text-xs text-muted-foreground px-4">
            ...
            return (
              ...
              >
                ...
                <span className="flex items-center gap-2 shrink-0">
                  <span className={cn('flex items-center gap-1 text-2xs font-mono font-bold', meta.cls)}>
                    <Icon className="h-3 w-3" />
                    {result.variancePct != null ? `${fmtNum(result.variancePct, 1)}%` : '0.0%'}
                  </span>
