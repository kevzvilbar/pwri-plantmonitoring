...
export function useDataSummaryData({
  ...
  const summaryStats = useMemo(() => {
    ...
    const avgProdCost = prodCostRows.length > 0
      ? prodCostRows.reduce((s, r) => s + (r.totalCost ?? 0), 0) / prodCostRows.length : null;
    const avgPowerCost = powerCostRows.length > 0
      ? powerCostRows.reduce((s, r) => s + (r.powerCost ?? 0), 0) / powerCostRows.length : null;
    const avgChemCost = chemCostRows.length > 0
      ? chemCostRows.reduce((s, r) => s + (r.chemCost ?? 0), 0) / chemCostRows.length : null;
    ...
    const avgRecovery = recoveryRows.length > 0
      ? recoveryRows.reduce((s, r) => s + r.recovery, 0) / recoveryRows.length : null;
    const minRecovery = recoveryRows.length > 0 ? Math.min(...recoveryRows.map((r) => r.recovery)) : null;
    const maxRecovery = recoveryRows.length > 0 ? Math.max(...recoveryRows.map((r) => r.recovery)) : null;
    const recoveryDays = recoveryRows.length;

    const tdsRows = overviewChartRows.filter((r) => r.tds != null && r.tds > 0);
    const avgTds = tdsRows.length > 0 ? tdsRows.reduce((s, r) => s + r.tds, 0) / tdsRows.length : null;
    ...
    return {
      totalProd, totalCons, totalRaw,
      avgDailyProd, avgDailyCons, avgDailyRaw,
      peakProd: peakRow?.production ?? 0,
      peakDate: peakRow?.date ?? '—',
      peakRaw, nrwPct,
      totalSolar, totalGrid, totalKwh, solarPct,
      gridPvRatio, totalPvRatio,
      avgProdCost, avgPowerCost, avgChemCost, totalCostOutput,
      avgRecovery, minRecovery, maxRecovery, recoveryDays,
      avgTds, minTds, maxTds, tdsDays,
    };
