...
async function fetchPending(): Promise<FlaggedRow[]> {
  ...
  for (const table of tables) {
    ...
    const entityIds = [...new Set(rows.map((r: any) => r[entityCol]))].filter(Boolean);
    ...
    // Resolve plant names
    const plantIds = [...new Set(rows.map((r: any) => r.plant_id))].filter(Boolean);
    ...
    const userIds = [...new Set(rows.map((r: any) => r.recorded_by))].filter(Boolean);
