...
import { Upload, Download, FileText, AlertCircle, Loader2, X, ChevronDown, Pencil, History, Trash2, BarChart2, Calendar, TrendingUp, ChevronLeft, ChevronRight, MessageCircleOff, MessageSquarePlus } from 'lucide-react';
...
import {
  // constants
  KNOWN_CHEMICALS, CHEM_UNITS, DOSING_KEYS,
  DEFAULT_CIP_CHEMICALS, CIP_BUILTIN_DB_MAP, CIP_CHEM_ACCENTS, CIP_CUSTOM_ACCENT,
  // csv
  parseROCSVText, normalizeRODatetime, validateROTrainRow,
  RO_TRAIN_SCHEMA, RO_TRAIN_TEMPLATE_ROW,
  // helpers
  Sparkline, TWO_HOURS_MS, deriveTrainStatus,
  EDIT_WINDOW_HOURS, canEditEntry, diffFields, logReadingEdit,
  recalculateTrainDeltas,
  // submit
  insertROTrainReadings, getPermeateDayLabel,
  type ConflictMode,
  // components
...
function Overview() {
  ...
  const { data: lastReadings } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('ro_train_readings').select('*')
        .in('train_id', trainIds).order('reading_datetime', { ascending: false });
      const map: Record<string, any> = {};
      for (const r of data ?? []) { if (!map[r.train_id]) map[r.train_id] = r; }
      return map;
    },
