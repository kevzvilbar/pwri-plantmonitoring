...
describe('useAlertEvents unit tests (P3-2 shared tablet outbox)', () => {
  ...
  describe('adopting the pre-partition shared outbox', () => {
    // Before user_id-keyed queues, every device wrote to one shared key
    // (getOutboxKey(null/undefined)). A row left there when this shipped must
    // still reach the server: flushAlertEventOutbox is only ever called with a
    // signed-in user's id, which reads `${OUTBOX_KEY_PREFIX}<uid>`, never the
    // old shared key — without adoption, that row is not lost to an error, it
    // is just never read again.
    it('a row already tagged with its owner moves to that owner\u2019s queue and is sent on their flush, not the current user\u2019s', async () => {
      const bobRow = { alert_key: 'dp-2', action: 'resolved' as const, note: 'fixed', user_id: 'user-bob' };
      writeOutbox(null, [bobRow]);

      const mockInsert = vi.fn().mockResolvedValue({ error: null });
      (supabase.from as any).mockReturnValue({ insert: mockInsert });

      // Alice signs in first and flushes; Bob's row is adopted into Bob's
      // queue, not sent under Alice's session.
      const sentForAlice = await flushAlertEventOutbox('user-alice');
      expect(sentForAlice).toBe(0);
      expect(mockInsert).not.toHaveBeenCalled();
      expect(readOutbox(null)).toHaveLength(0); // the shared key is drained either way
      expect(readOutbox('user-bob')).toEqual([bobRow]);

      const sentForBob = await flushAlertEventOutbox('user-bob');
      expect(sentForBob).toBe(1);
      expect(mockInsert).toHaveBeenCalledWith(bobRow);
    });

    it('a row with no owner at all (queued before user_id was written) is adopted by whoever is signed in now', async () => {
      const orphanRow = { alert_key: 'old-key', action: 'acknowledged' as const };
      writeOutbox(null, [orphanRow]);
      ...
      expect(sent).toBe(1);
      expect(mockInsert).toHaveBeenCalledWith(orphanRow);
      expect(readOutbox(null)).toHaveLength(0);
    });

    it('merges into an owner\u2019s EXISTING queue rather than overwriting it', async () => {
      const existing = { alert_key: 'already-queued', action: 'acknowledged' as const, user_id: 'user-alice' };
      const legacy = { alert_key: 'from-legacy', action: 'acknowledged' as const, user_id: 'user-alice' };
      writeOutbox('user-alice', [existing]);
      writeOutbox(null, [legacy]);
      ...
      expect(sent).toBe(2);
      expect(mockInsert).toHaveBeenCalledWith(existing);
      expect(mockInsert).toHaveBeenCalledWith(legacy);
    });

    it('does nothing when the shared key is empty (the common case, after the first adoption)', async () => {
      writeOutbox('user-alice', [{ alert_key: 'k', action: 'acknowledged' as const, user_id: 'user-alice' }]);
      const mockInsert = vi.fn().mockResolvedValue({ error: null });
      (supabase.from as any).mockReturnValue({ insert: mockInsert });

      await flushAlertEventOutbox('user-alice');
      mockInsert.mockClear();
      await flushAlertEventOutbox('user-alice');

      expect(mockInsert).not.toHaveBeenCalled(); // nothing left to adopt or send
    });
  });
});
