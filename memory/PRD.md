...
- [x] Mambaling Q2 & Umapad Q2 Downtime sheets → Supabase `downtime_events`.
...
### Architecture decision — 2026-07-22: §7.3 Mother Meter / Locators derived-meter (implemented)

**Decision:** implement the residual-method derived meter for the Hamas case.

**What was built:**

| Layer | Change |
|---|---|
| Schema | `locators.is_derived`, `locators.derived_from_meter_id`, `product_meters.is_derived`, `product_meters.derived_from_locator_id`, `derived_meter_sweep_log` audit table, `is_estimated` on both readings tables |
| Backend | `run_derived_meter_sweep()` in `cron_service.py`; `POST /api/cron/derived-meter-sweep` route |
| Frontend | `AssignLocatorsDialog` extended with "Has physical meter" Switch per locator + "Counts as production elsewhere?" mirror-target picker (plant + meter); derived locators shown with `~` prefix and amber badge in the meter card |
| Notifications | Silent recalculations fire one notification row per Admin on both affected plants via the existing `notifications` table |

**Open decisions resolved:**
- Sweep cadence: every 30–60 min (no explicit schedule in config.toml yet — add when ready to go live)
- Notification recipients: all Admins on both the source plant and the mirror target plant

**Constraints enforced:**
- At most one derived locator per mother meter (DB: partial unique index; UI: Switch disabled with error toast if already at limit)
- Admin-only to configure (both `AssignLocatorsDialog` and the mirror-target picker sit behind the existing `canEdit = isAdmin` guard in `ProductMetersCard`)

**§9.3 dead-code cluster — resolved 2026-07-22:**
- `useWells.ts`, `useLocators.ts`, `useROTrains.ts` — deleted (zero callers; the refactor that was supposed to replace inline Supabase queries never happened; §9.3 decision: delete)
- `AIImportPanel.tsx` — already gone from repo before this session
