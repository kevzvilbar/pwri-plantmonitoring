...
> This is the pre-2026-09-19 version of `DEPLOYMENT.md`, preserved verbatim for
> historical reference (MongoDB→Supabase migration tables, FastAPI/Railway
> removal narrative, and the original feature-setup walkthroughs). The current,
> consolidated guide is at `DEPLOYMENT.md` in the repo root. Note that some
> paths in here are stale (e.g. `frontend/supabase/migrations/…` — migrations
> have lived at `supabase/migrations/` since the 2026-09 squash).
