...
export async function findExistingReading(opts: {
  table: 'ro_train_readings' | 'afm_readings' | 'locator_readings' | 'well_readings' | 'power_readings' | 'cartridge_readings' | 'pump_readings';
  entityCol: string;          // e.g. 'train_id', 'locator_id', 'well_id', 'plant_id'
  entityId: string;
  datetime: Date;
  windowKind: 'hour' | 'day';
  ...
  let q = supabase.from(table).select('id').eq(entityCol, entityId)
    .gte('reading_datetime', start.toISOString())
    .lt('reading_datetime', end.toISOString())
    .limit(1);
  if (extraEqual) {
    for (const [k, v] of Object.entries(extraEqual)) q = q.eq(k, v as any);
  }
  const { data } = await q;
