...
def service_role_client() -> Client:
    ...
    RELIABILITY FIX (T3): this previously fell back to user_scoped_client()
    with NO access_token attached when the service-role key was missing —
    not a graceful degradation, but a guaranteed no-op. An anon client with
    no JWT has auth.uid() = NULL, which fails every RLS policy that checks
    "auth.uid() IS NOT NULL" (which is all of them). The delete would return
    HTTP 200 with zero rows actually affected, giving a false "success" to
    both the admin and the audit log. Fail loudly instead so the caller sees
    a 500 and knows the operation did not happen.
    """
    url = os.environ.get("SUPABASE_URL")
    service_key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY")
    if not url or not service_key:
        log.error(
            "SUPABASE_SERVICE_ROLE_KEY is not set — refusing to perform a "
            "privileged operation with a silently-downgraded client."
        )
        raise HTTPException(
            status_code=500,
            detail="Server is not configured for this operation (missing service role key). Contact an administrator.",
        )
