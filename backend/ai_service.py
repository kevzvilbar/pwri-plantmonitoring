...
async def get_session(db, session_id: str, caller: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    """Fetch one chat session's messages.

    `caller` is the resolved identity (user_id + roles) of the authenticated
    request. If the session has a recorded owner, only that owner or an
    Admin may read it — otherwise any authenticated caller could pull any
    other user's AI conversation just by guessing/enumerating session_id.
    """
    sb = _supa_client()
    if sb is None:
    ...
    try:
        ...
        if owner_id and caller and owner_id != caller.get("user_id") and "Admin" not in (caller.get("roles") or []):
