...
async def run_compliance_evaluate(db) -> dict[str, Any]:
    ...
    for p in plants:
        ...
        # NOTE: "nrw_pct" has no source column anywhere in the schema (daily_plant_summary
        # has production_m3 / locator_consumption_m3 / raw_water_consumption_m3 / blending_m3
        # instead) — this key will always resolve to None until NRW% is actually computed
        # from those columns. Flagging rather than guessing at the formula here.
        metrics = {k: latest.get(k) for k in ("nrw_pct","downtime_hrs","permeate_tds","permeate_ph","raw_turbidity_ntu","dp_psi","recovery_pct","pv_ratio")}
