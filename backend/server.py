...
from supa_client import _client as _supa_client
...
# ── Startup env-var validation (§4 item 7) ────────────────────────────────────
# Fail fast with a clear message rather than surfacing as a runtime 500 on
# whichever route happens to touch a missing variable first.
...
def _validate_env() -> None:
    ...
    if not os.environ.get("SUPABASE_ANON_KEY") and not os.environ.get("SUPABASE_SERVICE_ROLE_KEY"):
        missing.append("SUPABASE_ANON_KEY (or SUPABASE_SERVICE_ROLE_KEY)")
    if missing:
        raise SystemExit(
            "\n[PWRI] Missing required environment variables:\n"
            + "".join(f"  • {v}\n" for v in missing)
            + "Set them in backend/.env or your deployment environment.\n"
        )
    # Warn (not fatal) for optional-but-needed-at-runtime vars
    optional_warned = {
        "SUPABASE_SERVICE_ROLE_KEY": "admin/hard-delete routes will return 500",
        "EMERGENT_LLM_KEY":          "AI chat/anomaly endpoints will return errors",
        "CRON_SECRET":               "cron routes will reject all calls",
    }
    for var, impact in optional_warned.items():
        if not os.environ.get(var):
            log.warning("[PWRI] %s not set — %s", var, impact)
...
def supa() -> Optional[Client]:
    """Anon Supabase client. Delegates to supa_client._client() so there is
    exactly one client-creation path across the backend."""
    return _supa_client()
...
# ── Rate limiting ────────────────────────────────────────────────────────────
# AI endpoints proxy to a paid external LLM API. Combined with S1's auth fix
# this bounds both abuse (spam) and cost exposure (unbounded LLM calls) from
# a single caller. Per-IP rather than per-user so it also protects against
# many-account abuse.
...
async def ai_chat(request: Request, req: ChatRequest, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    try:
        return await chat_turn(None, caller["user_id"], req)
...
@limiter.limit(AI_RATE_LIMIT)
async def ai_list_sessions(request: Request, authorization: Optional[str] = Header(None), limit: int = 20):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    return await list_sessions(None, caller["user_id"], limit=max(1, min(limit, 100)))


@api_router.get("/ai/sessions/{session_id}")
@limiter.limit(AI_RATE_LIMIT)
async def ai_get_session(request: Request, session_id: str, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    return await get_session(None, session_id, caller=caller)


@api_router.delete("/ai/sessions/{session_id}")
@limiter.limit(AI_RATE_LIMIT)
async def ai_delete_session(request: Request, session_id: str, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    db = supa()
    if db:
...
@limiter.limit(AI_RATE_LIMIT)
async def ai_anomalies(request: Request, req: AnomalyRequest):
    try:
        return await detect_anomalies(req)
    except RuntimeError as e:
        raise HTTPException(502, str(e))
    except Exception as e:
        log.exception("ai_anomalies failed")
        raise HTTPException(500, str(e))


@api_router.post("/ai/chat-tools")
@limiter.limit(AI_RATE_LIMIT)
async def ai_chat_tools(request: Request, req: ChatWithToolsRequest, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    try:
        return await chat_with_tools(None, caller["user_id"], req)
    except RuntimeError as e:
        raise HTTPException(502, str(e))
    except Exception as e:
        log.exception("ai_chat_tools failed")
        raise HTTPException(500, str(e))
...
@limiter.limit(AI_RATE_LIMIT)
async def ai_pm_forecast(request: Request, req: PmForecastRequest):
    try:
        return await forecast_pm(req)
    except RuntimeError as e:
        raise HTTPException(502, str(e))
    except Exception as e:
        log.exception("pm_forecast failed")
        raise HTTPException(500, str(e))
...
async def put_compliance_thresholds(body: dict, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
...
async def compliance_evaluate(body: ViolationsPayload, summarize: bool = False,
    ...
    # Any authenticated user may run a compliance evaluation (read-mostly,
    # scoped to the caller's own submitted metrics) — this only requires a
    # valid session, not a specific role, unlike the threshold writes above.
    token = bearer_token(authorization)
    ...
    if summarize:
        try:
            ...
            log.exception("compliance summary failed")
...
async def import_seed_from_url(body: SeedRequest, authorization: Optional[str] = Header(None)):
    ...
    try:
        ...
        log.exception("seed_from_urls failed")
...
async def blending_toggle(body: BlendingToggleRequest, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    db = supa()
    if db is None:
    ...
    if body.is_blending:
        db.table("blending_wells").upsert({
            ...
            "tagged_by": caller["user_id"], "tagged_at": datetime.utcnow().isoformat(),
            "note": body.note or "",
...
async def blending_log_event(body: BlendingAuditRequest, authorization: Optional[str] = Header(None)):
    token = bearer_token(authorization)
    caller = caller_identity(token)
    require_roles(caller, {"Admin", "Manager"})
    db = supa()
...
async def alerts_feed(plant_id: Optional[str] = None, days: int = 7):
    ...
    if plant_id:
        q_dt = q_dt.eq("plant_id", plant_id)
    ...
    if plant_id:
        q_be = q_be.eq("plant_id", plant_id)
    ...
    if plant_id:
        q_snap = q_snap.eq("plant_id", plant_id)
    seen: set[str] = set()
        ...
        for v in s.get("violations") or []:
            if v.get("code") == "recovery_pct_under":
                alerts.append({"kind": "recovery", "severity": v.get("severity", "medium"),
                               ...
                               "detail": f"Recovery {v.get('value')}% vs. min {v.get('threshold')}%"})
                break
...
#
# RETIRED 2026-07-21 — §4 item 1 (master plan: consolidate duplicate regression
# implementations into one authoritative copy).
#
# All seven routes that used to live here were never called by the frontend:
#   GET  /data-analysis/tables
#   POST /data-analysis/run-regression
#   POST /data-analysis/apply-regression
#   POST /data-analysis/retract-regression
#   GET  /data-analysis/results
#   POST /data-analysis/edit-raw
#   GET  /data-analysis/raw-edit-log
#
# DataAnalysis.tsx (line 23) reads: "Backend: 100% Supabase — no Python backend
# required."  The live regression pipeline lives entirely in TypeScript:
#   runOLS()              — two-pass OLS with meter-reset pre-pass
#   handleApply()         — compare-and-swap apply with train delta cascade
#   handleRetract()       — compare-and-swap retract with original-value restore
#   detectGaps()          — gap-fill interpolation
#   raw_edit_log writes   — direct Supabase insert in EditRawDialog
#
# The Python implementation (`_retired_regression_service.py`) was a single-pass
# OLS with no race protection, no reset detection, and no train cascade — the
# inferior copy.  Keeping both was what caused bug D2 to be "verified fixed" on
# the Python side while the live TS path still had the defect.
#
# The retired file is kept in the repo for archaeology; it has no effect on the
# running application.
# ──────────────────────────────────────────────────────────────────────────────
...
async def import_ai_analyze(file: UploadFile = File(...), plant_id: Optional[str] = None,
    ...
    if len(data) > AI_MAX_FILE_BYTES:
        raise HTTPException(413, "File too large (limit 25 MiB)")
...
async def cron_compliance_evaluate(x_cron_secret: Optional[str] = Header(None)):
    ...
    try:
        ...
        log.exception("cron compliance failed")
        raise HTTPException(500, str(e))
...
async def cron_pm_forecast_sweep(x_cron_secret: Optional[str] = Header(None)):
    ...
    try:
        ...
        log.exception("cron pm sweep failed")
        raise HTTPException(500, str(e))
...
async def admin_create_user(body: AdminCreateUserRequest, authorization: Optional[str] = Header(None)):
    ...
    if not uid:
        raise HTTPException(500, "Admin API returned no user ID.")
...
app.add_middleware(
    ...
    allow_headers=["Authorization", "Content-Type", "X-User-Id", "X-Cron-Secret"],
)
